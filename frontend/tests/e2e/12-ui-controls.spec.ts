...
test('UI-02: タスク削除ボタンは削除権限を持つ作成者にのみ表示される', async ({ page }) => {
...
test('UI-03: コメント編集ボタンは投稿者本人にのみ表示される', async ({ page }) => {
  const assignee = createE2eUser()
...
test('UI-04: コメント削除ボタンは投稿者本人にのみ表示される', async ({ page }) => {
  const assignee = createE2eUser()
  const commenter = createE2eUser()
...
test('UI-06: 添付削除ボタンは添付登録者本人にのみ表示される', async ({ page }) => {
  const assignee = createE2eUser()
