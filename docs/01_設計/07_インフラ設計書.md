...
### 1.3 対象外
- コメント機能
- 添付ファイル機能の詳細設計（別紙「16_添付ファイル詳細設計書」に定義。本書では実行基盤上の保存先切替前提のみ扱う）
- チーム管理 / 通知 / 管理者画面
...
- 添付ファイル保存は backend のストレージプロバイダ設定で切り替え、本番運用では `S3`、ローカル開発・自動テストでは既定値 `LOCAL` を使用する
...
#### 添付ファイル保存方式
- 実装既定値は `APP_ATTACHMENT_STORAGE_TYPE` 未指定時 `LOCAL`
- 本番・ステージングで添付ファイルを永続保存する場合は `APP_ATTACHMENT_STORAGE_TYPE=S3` を明示する
- S3利用時は `APP_ATTACHMENT_S3_BUCKET`、`APP_ATTACHMENT_S3_REGION`、`APP_ATTACHMENT_S3_PREFIX` を環境に応じて設定する
- `LOCAL` はローカル開発・自動テスト向けの既定値であり、本番の永続保存先としては使用しない
...
- 本項のS3バケットは frontend 配信用であり、添付ファイル保存用S3バケットとは共有しない。
- 添付ファイル保存用S3バケットの詳細は「16_添付ファイル詳細設計書」に従う。
