...
| 1.2 | 2026-04-28 | チーム管理機能に伴う teams、team_members、tasks.team_id、関連リレーション・制約・インデックスを追加 | 佐伯 |
...
- `users.role` はシステム全体ロール、`team_members.role` はチーム内ロールとして分離する
- ユーザーは複数チームに所属可能とする
- チーム内ロールは `OWNER` / `ADMIN` / `MEMBER` とする
- タスクは必ず1つのチームに属する
- タスク担当者は、当該タスクと同一チームに所属するユーザーのみ設定可能とする
- コメント、添付ファイル、通知、アクティビティログには `team_id` を冗長保持せず、親タスク経由でチームを判定する
...
- チーム管理操作は `activity_logs` には保持せず、構造化ログの audit ログで追跡する
...
| テーブル名 | 論理名 | 概要 |
|---|---|---|
| users | ユーザー | タスク管理システムの利用者情報とシステム全体権限を保持する |
| teams | チーム | チーム名、説明、作成者を保持する |
| team_members | チームメンバー | チーム所属ユーザーとチーム内ロールを保持する |
| tasks | タスク | タスク本体、所属チーム、担当者、作成者を保持する |
...
タスク情報を保持するテーブル。  
タイトル、説明、状態、優先度、期限日、所属チーム、担当者、作成者を管理する。
...
| fk_tasks_team | 外部キー | team_id → teams.id |
...
| idx_tasks_team_id | team_id | チーム単位タスク検索用 |
| idx_tasks_team_status | team_id, status | チーム単位タスク一覧のステータス絞り込み用 |
| idx_tasks_team_assigned_user | team_id, assigned_user_id | チーム内担当者絞り込み用 |
...
- タスクは必ず1つのチームに属する
- タスク作成後の `team_id` 変更は本フェーズでは不可とする
- `assigned_user_id` を設定する場合、対象ユーザーは当該 `team_id` に所属している必要がある
- `assigned_user_id` と `team_id` の整合性は、複合外部キーではなくService層のアプリケーションロジックで担保する
- 親タスク削除時は、紐づくコメントと添付ファイルメタ情報を連鎖論理削除対象とする
- `activity_logs` と `notifications` は、親タスク削除時も削除しない
...
| No | カラム名 | データ型 | PK | UK | NN | FK | 説明 |
|---|---|---|---|---|---|---|---|
| 1 | id | BIGSERIAL | ○ |  | ○ |  | チームID |
| 2 | name | VARCHAR(100) |  |  | ○ |  | チーム名 |
| 3 | description | TEXT |  |  |  |  | チーム説明 |
| 4 | created_by | BIGINT |  |  | ○ | ○ | 作成者ユーザーID |
...
|---|---|---|
| teams_pkey | 主キー | id |
| fk_teams_created_by | 外部キー | created_by → users.id |
| uk_teams_created_by_name | 一意制約 | created_by, name |

### 5.7.4 インデックス

| インデックス名 | 対象カラム | 説明 |
|---|---|---|
| idx_teams_created_by | created_by | 作成者単位の検索用 |

### 5.7.5 補足

- 同一作成者による同名チーム作成は不可とする
- チーム作成時、作成者を `team_members` に `OWNER` として登録する
- `teams` には論理削除カラムを持たせない
- チーム削除は本フェーズ対象外とする

</details>

<details>
<summary>5.8 team_members</summary>

## 5.8 team_members

### 5.8.1 概要

チーム所属ユーザーとチーム内ロールを保持する中間テーブル。  
...
| No | カラム名 | データ型 | PK | UK | NN | FK | 説明 |
|---|---|---|---|---|---|---|---|
| 1 | id | BIGSERIAL | ○ |  | ○ |  | チームメンバーID |
| 2 | team_id | BIGINT |  |  | ○ | ○ | 所属チームID |
| 3 | user_id | BIGINT |  |  | ○ | ○ | 所属ユーザーID |
| 4 | role | VARCHAR(20) |  |  | ○ |  | チーム内ロール |
| 5 | joined_at | TIMESTAMP |  |  | ○ |  | 参加日時 |

### 5.8.3 制約

| 制約名 | 種別 | 内容 |
|---|---|---|
| team_members_pkey | 主キー | id |
| fk_team_members_team | 外部キー | team_id → teams.id |
| fk_team_members_user | 外部キー | user_id → users.id |
| uk_team_members_team_user | 一意制約 | team_id, user_id |
| chk_team_members_role | CHECK制約 | role in (`OWNER`, `ADMIN`, `MEMBER`) |

### 5.8.4 インデックス

| インデックス名 | 対象カラム | 説明 |
|---|---|---|
| idx_team_members_team_id | team_id | チーム単位メンバー一覧取得用 |
| idx_team_members_user_id | user_id | ユーザー所属チーム一覧取得用 |
| idx_team_members_team_role | team_id, role | チーム内ロール判定補助用 |
| uk_team_members_owner_per_team | team_id | `role = 'OWNER'` の場合に1チーム1OWNERを担保する部分ユニークインデックス |

### 5.8.5 補足

- 同一ユーザーの同一チーム重複所属は不可とする
- チーム作成時、作成者を `OWNER` として1件追加する
- 主キーは `(team_id, user_id)` の複合主キーではなく、サロゲートキー `id` を採用する
- チームメンバー削除時は `team_members` のレコードを物理削除する
- `OWNER` の最大1名制約は、`uk_team_members_owner_per_team` 部分ユニークインデックスでDBにより担保する
- `OWNER` の最低1名は、アプリケーションロジックおよび運用で担保する
- OWNER移譲機能は本フェーズ対象外とする
...
- `users.role` はシステム全体ロールとして扱う
- チーム単位の操作可否は `team_members.role` を主軸に判定する
...
- `OWNER` / `ADMIN` / `MEMBER` はチーム内の操作権限を表す
- `users.role` とは役割を分離する
...
## 7.10 users - teams（作成者）

| 項目 | 内容 |
|---|---|
| 親テーブル | `users` |
| 子テーブル | `teams` |
| 外部キー | `teams.created_by` |
| 参照先 | `users.id` |

1ユーザーは複数チームを作成できる。

## 7.11 teams - team_members

| 項目 | 内容 |
|---|---|
| 親テーブル | `teams` |
| 子テーブル | `team_members` |
| 外部キー | `team_members.team_id` |
| 参照先 | `teams.id` |

1チームは複数メンバーを持てる。

## 7.12 users - team_members

| 項目 | 内容 |
|---|---|
| 親テーブル | `users` |
| 子テーブル | `team_members` |
| 外部キー | `team_members.user_id` |
| 参照先 | `users.id` |
...
- アプリケーション側で自動設定する
- `teams` の `created_at` / `updated_at` は共通の作成・更新日時として扱う
...
- `teams` と `team_members` には削除系カラムを持たせない

## 8.3 チーム参加日時カラム

| カラム名 | データ型 | 説明 |
|---|---|---|
| joined_at | TIMESTAMP | チーム参加日時 |

### 補足

- `team_members.joined_at` はチーム参加時に設定される
- `team_members` には `created_at` / `updated_at` を持たせず、参加日時を `joined_at` で管理する
...
| teams | id | BIGSERIAL / IDENTITY |
| team_members | id | BIGSERIAL / IDENTITY |
...
| teams | 削除なし | チーム削除は本フェーズ対象外のため |
| team_members | 物理削除 | チームから外れた事実は構造化ログで追跡し、所属状態は現時点の状態を保持するため |
...
- チーム管理操作は `activity_logs` には保持せず、構造化ログで追跡する

## 9.5 既存データ移行方針

既存データが存在する前提で、段階的に移行する。

### 想定ステップ

1. `teams` テーブルを作成する
2. `team_members` テーブルを作成する
3. `tasks.team_id` を追加する
4. 既存ユーザーごとに個人初期チームを作成する
5. 対象ユーザー本人を個人初期チームへ `OWNER` として登録する
6. 既存タスクを作成者の個人初期チームへ所属させる
7. 既存タスクの担当者が作成者と異なる場合は、担当者も当該チームへ `MEMBER` として所属させる
8. `tasks.team_id IS NULL` の残件がないことを確認する

### 個人初期チーム

- 個人初期チーム名は `<ユーザー名>のチーム` を基本とする
- description は空とする
- 初回ログイン時のチーム名変更フローは本フェーズでは扱わない

### 移行後確認観点

- `tasks.team_id IS NULL` の残件がないこと
- `tasks.assigned_user_id` と `team_members(team_id, user_id)` の不整合がないこと
- `team_members` において、team ごとの `OWNER` が複数件になっていないこと
- `team_members` において、team ごとの `OWNER` が0件になっていないこと
...
既存DBに対して適用する場合は、`teams` / `team_members` 作成後に既存データへ `tasks.team_id` を設定し、未設定データが残っていないことを確認してから `NOT NULL` 制約を有効化する。
...
- `team_members.role` はチーム内ロールとして定義し、`users.role` とは役割を分離する
- チーム管理機能では、`teams`、`team_members`、`tasks.team_id` を追加する
- `OWNER` の最大1名制約は、`team_members` の `uk_team_members_owner_per_team` 部分ユニークインデックスでDBにより担保する
- `OWNER` の最低1名は、アプリケーションロジックおよび運用で担保する
- コメント、添付ファイル、通知、アクティビティログには `team_id` を冗長保持せず、親タスク経由でチームを判定する
- 添付ファイルの保存先をS3にする場合、インフラ設計書および非機能要件定義書との整合確認を行う
- 本書更新後、API設計書、画面設計書、エラー設計書、認可設計書との整合確認を行う
- Flywayマイグレーションの詳細な移行SQL、確認SQL、ロールバック方針は、マイグレーション設計または実装タスクで具体化する
