...
| 6 | created_at | TIMESTAMP |  |  | ○ |  | 作成日時 |
| 7 | updated_at | TIMESTAMP |  |  | ○ |  | 更新日時 |
| 8 | deleted_at | TIMESTAMP |  |  |  |  | 削除日時 |
| 9 | deleted_by | BIGINT |  |  |  | ○ | 削除者ID |
...
- コメント削除は論理削除とする
- 論理削除済みコメントを画面で非表示にするか、削除済み表示にするかは画面設計で定義する
...
CREATE TABLE notifications (
    ...
    activity_log_id BIGINT NOT NULL REFERENCES activity_logs(id),
