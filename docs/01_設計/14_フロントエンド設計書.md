...
| 実行状態 | isSubmittingTask, isDeleting, taskConflictMessage |
...
- 対象はタスク保存 / 削除、コメント投稿 / 更新 / 削除、添付アップロード / 削除、通知既読化 / 一括既読化とする。
- タスク作成、コメント投稿、添付アップロードは `Idempotency-Key` を使わないため、画面側の `disabled` 制御で重複作成を抑止する。
...
- タスク更新で `ERR-TASK-007` を受けた場合は、`他のユーザーによりタスクが更新されました。最新状態を再読み込みしてください。` を表示する。
