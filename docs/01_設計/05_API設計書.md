...
- 成功時は `success` フラグ付きの共通 envelope ではなく、raw DTO / raw JSON 配列 / ボディなしを返す
- 一覧系は JSON 配列、単票系は API ごとの DTO、削除系は `204 No Content` を基本とする
- 正常系と異常系で責務を分け、失敗時のみ `ErrorResponse` を共通形式として扱う
- 今後 API を追加する場合も、現時点ではこの方針を踏襲する

### 方針採用理由

- 現行 API 数では、成功レスポンスまで共通ラッパーへ統一するメリットよりも、DTO をそのまま扱える単純さを優先できるため
