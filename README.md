...
単なる CRUD 実装で終わらせず、**JWT 認証、チーム単位の認可制御、例外設計、AWS 上での公開、GitHub Actions + OIDC による本番デプロイ自動化、CloudWatch Logs を用いたログ追跡**まで一通り実装しています。
...
- 公開環境で主要導線を確認済み
  ...
  - ログイン
  - チーム一覧 / 詳細
  - チーム作成 / メンバー追加 / ロール変更 / メンバー削除
  - タスク一覧 / 詳細 / 作成 / 更新 / 削除
  - 所属全チーム横断のタスク一覧 / チーム文脈付きタスク一覧
  - コメント / 添付 / 通知
  - 通知ポーリング
  - アクティビティログ表示
  - ログアウト
...
- チーム内ロール変更
- チームメンバー削除
- OWNER / ADMIN / MEMBER によるチーム内ロール管理
- タスク一覧表示
- 所属全チーム横断のタスク一覧表示
- チーム文脈付きタスク一覧表示
- タスク詳細表示
...
![ログイン画面](docs/01_設計/image/画面設計書_ログイン画面.png)

### チーム一覧画面

![チーム一覧画面](docs/01_設計/image/画面設計書_チーム一覧画面.png)

### チーム詳細画面

![チーム詳細画面](docs/01_設計/image/画面設計書_チーム詳細画面.png)

### タスク一覧画面

![タスク一覧画面](docs/01_設計/image/画面設計書_タスク一覧画面.png)

### タスク詳細画面

![タスク詳細画面](docs/01_設計/image/画面設計書_タスク詳細画面_表示.png)
...
- 認証は JWT ベースで実装
- 認可は「ログインしているか」だけでなく、**チームに所属しているか、チーム内でどのロールか、対象タスクに対して操作できるか**まで含めて判定
- チーム内ロールは `OWNER / ADMIN / MEMBER` とし、チーム管理操作とタスク操作の表示・API認可に利用
- 未認証、不正トークン、他人チーム・他人タスクへの参照・更新・削除拒否を確認できる構成

現状の認可ルールは、**チーム所属 + チーム内ロール + タスクの作成者 / 担当者**を組み合わせています。システム全体の管理者ロールは将来拡張として切り分け、チーム内の業務権限は `team_members.role` を中心に扱っています。

### 2. チーム境界を導入し、タスクをチーム単位で扱えるようにしている

- ユーザーは複数チームに所属可能
- タスクは必ず1つのチームに所属
- チーム一覧 / チーム詳細 / メンバー管理 / ロール変更を実装
- `/tasks` は所属全チーム横断、`/tasks?teamId=...` はチーム文脈付き一覧として使い分け
- タスク作成はチーム文脈から行い、担当者候補は対象チームのメンバーに限定

単なる個人タスク管理ではなく、**複数人・複数チームで使う前提の権限境界とデータ境界**を持たせています。
...
ローカルと本番でスキーマ差分を管理しやすく、テーブル変更履歴を追える構成にしたかったため採用しました。チーム管理機能では `teams`、`team_members`、`tasks.team_id` を Flyway マイグレーションで追加し、1チーム1OWNERの制約もDB側の部分ユニークインデックスで担保しています。
...
- 認証、認可、バリデーション、タスク CRUD、チーム管理の代表ケースを対象に自動確認
...
### 4. チーム管理機能追加時に、既存タスク機能を壊さず段階的に拡張

既存のタスク管理をチーム単位へ拡張する際、単にチーム画面を増やすだけではなく、**タスク一覧、タスク作成、担当者候補、認可、DB制約、画面遷移**が一貫するように調整しました。  
`/tasks` は所属全チーム横断、`/tasks?teamId=...` はチーム文脈付き一覧として扱い、タスク作成はチーム詳細からの導線に限定することで、チーム未確定のタスクが作られないようにしています。

これらの対応を通じて、単に機能を実装するだけでなく、**公開後に起こる不整合を切り分けて直す力と、既存機能を保ちながら拡張する設計力**も示せるようにしました。
